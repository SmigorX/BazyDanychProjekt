...
def create_user(user_data: NewUserSchema):
    conn = get_connection()
    email_check_query = text("SELECT COUNT(*) FROM Users WHERE email = :email")
...
def update_user_password(user_data: UpdateUserPasswordSchema):
    conn = get_connection()
    try:
        password_id_query = text("SELECT password_id FROM Users WHERE email = :email AND is_deleted = FALSE")
...
def get_user_data(userid: str) -> GetUserDataSchema | None:
    ...
    try:
        query = text("""
            SELECT id, email, first_name, last_name, picture_url, description
            FROM Users
...
def delete_user(userid: str):
    ...
    try:
        query = text("""
            ...
            WHERE email = :email
        """)
        conn.execute(query, {"email": userid})
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def update_user_email(user_data: dict):
    conn = get_connection()
    try:
        check_query = text("SELECT COUNT(*) FROM Users WHERE email = :new_email")
        if conn.execute(check_query, {"new_email": user_data['new_email']}).scalar() > 0:
            raise Exception("Ten email jest już przypisany do innego konta")
